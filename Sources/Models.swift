...
enum Fmt {
    ...
    ///
    /// Never "0m", and `Int(_: Double)` is not handed anything it can trap on. The seconds
    /// reaching here come from dates the API sent, and nothing stops that endpoint from
    /// putting the year 90000 in a `resets_at` - `Int(1e30)` is a crash, not a large
    /// number. Same rule as `clampPercent`, at the other end of the app.
