...
Resources/
  ...
  AppIcon.icns         the app icon, generated by `make icon` and committed
Tools/
  MakeIcon/main.swift  draws the icon from the mark (only run by `make icon`)
Tests/
  main.swift           331 logic checks, `make test`
...
Makefile               build / test / icon / install / run / zip / clean / uninstall
```

### The icon

<img src="docs/app-icon.png" width="128" alt="The app icon: the white Claude mark on an indigo plate">

`make icon` renders it from `Resources/claude-mark.png` and writes both
`Resources/AppIcon.icns` (the ten sizes `iconutil` wants) and `docs/app-icon.png`. The
`.icns` is committed, so a normal build never runs the generator; only editing the mark or
`Tools/MakeIcon/main.swift` calls for it.

The plate is indigo rather than Claude's terracotta on purpose: this is an unofficial app,
and wearing Anthropic's brand color in the Dock would suggest otherwise. The geometry is
Apple's macOS grid - an 824 pt rounded square on a 1024 pt canvas, corner radius 185.4 -
so it lines up with the stock icons around it.

`make test` compiles every source except `App.swift` together with `Tests/main.swift` into
a plain command line binary and runs it: 331 checks in about a second. No XCTest and no
...
and how they are sorted, how the desktop app's virtual machine is told apart from any
other one on the system, the severity thresholds, date and money formatting, the rows
